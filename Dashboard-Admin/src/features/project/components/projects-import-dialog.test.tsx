...
describe('ProjectsImportDialog', () => {
  ...
  it('renders the dialog with the correct title, description, file input and buttons', async () => {
    ...
    const { getByRole, getByText, getByLabelText } = await render(
      <ProjectsImportDialog open onOpenChange={onOpenChange} />
    )

    const title = getByRole('heading', {
      level: 2,
      name: /Import Projects/i,
    })
    const desc = getByText('Import projects quickly from a CSV file')
  ...
  it('calls showSubmittedData and closes when a CSV file is imported', async () => {
    ...
    const csv = new File(['a,b'], 'projects.csv', { type: 'text/csv' })
