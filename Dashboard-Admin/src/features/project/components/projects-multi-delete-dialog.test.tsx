...
describe('ProjectsMultiDeleteDialog', () => {
  ...
  it('renders the dialog with the correct title, description, input and buttons', async () => {
    ...
    const { getByRole, getByText } = await render(
      <ProjectsMultiDeleteDialog open onOpenChange={vi.fn()} table={table} />
    )

    const title = getByRole('heading', {
      level: 2,
      name: /Delete 2 projects/i,
    })
  ...
  it('closes the dialog when the cancel button is clicked', async () => {
    ...
    const { table } = createTableMock()
    const { getByRole } = await render(
      <ProjectsMultiDeleteDialog open onOpenChange={onOpenChange} table={table} />
    )
